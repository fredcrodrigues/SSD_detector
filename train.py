...
def l_image(string):
    trainNumpy = np.zeros((len(string) , row , col ))
    for i , t_string in enumerate(string):
        t_string = cv2.imread(t_string , 0)
        t_string = cv2.resize(t_string , dsize=(300,300) , interpolation=cv2.INTER_CUBIC)
        trainNumpy[i,:,:] = t_string
...
if __name__ == '__main__':
    ...
    for epoch in range(load_weights_from_epoch + 1, EPOCHS):
        ...
        for step, batch_data in enumerate(train_data):
            ...
            print("Epoch: {}/{}, step: {}/{}, {:.4f}s/step, loss: {:.4f}, "
                  "cls loss: {:.4f}, reg loss: {:.4f}".format(epoch,
                                                              EPOCHS,
                                                              step,
        ...
        #conv  = loss_metric.result().numpy()
        #print("LOSS" ,  conv)  
        #print("LOSS" , conv.astype('int'))  
        #plt.figure(figsize=[8,6])
        #plt.title("LOSS" , fontsize=16)
        #plt.plot(history.conv, 'r' , linewidth=3.0)
